...
|---|---|---|---|---|
| [`dsh-todo`](plugins/dsh-todo) | [npm](https://www.npmjs.com/package/@dennisrongo/dsh-todo) | per-workspace sprint/roadmap task list, plus a `dsh-todo` CLI | host + client + CLI | `dshTodo/list`, `replace`, `scanDigest`, `readSuggestions` |
...
| [`dsh-hooks`](plugins/dsh-hooks) | [npm](https://www.npmjs.com/package/@dennisrongo/dsh-hooks) | Claude Code-compatible hook lifecycle — shell commands at eight lifecycle points, plus contextual skill hints beside the composer | host + client | `dshHooks/describe`, `recent`, `hints`, `hintsToken`, `dismissHint` |
| [`dsh-plan-board`](plugins/dsh-plan-board) | [npm](https://www.npmjs.com/package/@dennisrongo/dsh-plan-board) | durable plans — captures every `exit_plan_mode` plan to disk, opens a review panel you can approve or revise from, keeps the history | host + client | `dshPlans/list`, `get`, `changeToken`, `discard`, `pin` |
...
**Suggest.** A button in the tab header scans the workspace and proposes concrete next tasks — unresolved `TODO`/`FIXME`/`HACK` comments, features the docs promise but the code does not implement, and modules with no tests — each with a rationale, a priority and a `file:line` pointer where there is one. Tick the ones worth doing and **Add selected** files them into the backlog; until then they are proposals, stored nowhere. **Refresh** returns genuinely new ideas because every title already shown joins the exclusion set alongside the existing backlog. The scan runs as a background session that writes a result file the modal polls, on a bounded digest rather than the repository — so it spends tokens, and it is never automatic.
...
**Endpoints.** `POST /api/dshTodo/list` → `{ list: { items, revision, updatedAt } }`; `POST /api/dshTodo/replace` → the new list or a revision conflict; `POST /api/dshTodo/scanDigest` → the bounded workspace digest a scan reasons over; `POST /api/dshTodo/readSuggestions` → `pending` until the scan's result file lands, then `ready` with the parsed suggestions or `error`, consuming the file either way. All four take a single parameter named `request`.
...
Current conditions pinned across the top of the web UI: temperature, condition, location, a short hourly outlook, humidity and wind. It centres on the span no docked overlay has claimed, so it moves aside for the plan panel and mission control instead of sitting under them.
...
**The same signals also suggest a skill.** A chip strip in `conversation.input.dock` names a skill you could run next — from what is on disk (a `.csproj`, a `next` dependency, a shadcn `components.json`), from a trigger phrase your prompt matched in a skill's own description, and from what the turn just did (three source edits and no test run, or a `git commit`). Clicking a chip writes `/skill-name ` into the composer, which is exactly what picking it from the slash menu does; `▶` runs it, `×` dismisses it for the session. Every rule carries a **pattern**, resolved against `ctx.skills.list()` for your agent's scope — so no chip ever names a skill this deployment does not have, and one you have already used is never suggested again. The heuristics are cheap regexes on purpose: a false positive is one ignorable chip, and a rule you can predict is one you can silence (`hints.disableRules`).

**Endpoints.** `POST /api/dshHooks/describe` → every hook in force across both layers with the document each came from; `POST /api/dshHooks/recent` → the last 200 settled runs with exit codes, durations and output tails; `hints` / `hintsToken` / `dismissHint` → the chip strip, where `hintsToken` is an in-memory counter that moves only when the computed list actually changes, so the 1.5s poll costs nothing on an idle session. All take a single parameter named `request`.

**Requires.** `ctx.tools` and `ctx.subprocess`. `ctx.settings`, `ctx.workspaceRegistry` and `ctx.skills` are used when present and deliberately not injected — this cordis has no optional-inject form, so listing them would make the plugin never mount in a deployment that composes none of them. Without a skill registry the strip simply stays empty.
...
**Two ways a plan is captured, because most sessions never enter plan mode.** Explicitly, from `exit_plan_mode`, which carries the real review outcome. Implicitly, from a fenced ````plan block in an assistant message (four backticks, so the plan's own code blocks cannot end it early) — a system-prompt section this plugin registers asks the model to mark plans that way, so detection is exact rather than a heuristic that would fire on any answer with a heading and a list. Unfenced plans are simply not captured, and every assistant message carries a **Pin as plan** action for that case. Implicit plans are stored as *Proposed*: no review was raised, so they are not labelled "Awaiting review".

**The plan docks beside the chat, taking its right half and pushing it aside** rather than covering it — closable, and only present while the session actually has a plan. It is a `shell.overlay` entry rather than a tab, and that is not a style choice: views render one-at-a-time (`only: <active id>`), so an inactive tab is not mounted and cannot open itself when a plan appears. But an overlay is not a layout sibling of the chat either, so the panel measures the conversation column and pushes it with an inline `padding-right`. It anchors on `[data-slot="conversation"]` because slot names are the documented API while the class names beside them are hashed per build, and a `MutationObserver` re-applies the push if a React re-render drops it. The Plans tab remains as the history browser.

**It approves without owning the question service.** `exit_plan_mode` presents through `ctx.userQuestions.ask()`, and that service documents **one active provider per context** — the shipped question UI holds it, and registering a second one would hijack every question in the harness, not just plan reviews. It never does. A raised question also reaches the browser as a pending-interaction carrier on the session's conversation snapshot, and the *carrier* owns the answer: the panel reads `sessions.binding(id).session.getSnapshot().pending`, narrows it with the same rules the shipped decision card uses, and calls `respond()`. Two remote controls for one wait — whichever answers first settles it, and the other gets a `not-pending` receipt it reports rather than swallows. **Revise** goes down the same channel: `exit_plan_mode` treats free text as keep-planning before it looks at the button, so an edited plan cannot be approved — it is sent back as feedback and presented again.
